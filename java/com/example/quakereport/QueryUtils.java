...
//            }
//        } catch(IOException e){
//            Log.e(LOG_TAG, "Problem retrieving the earquake Json results", e);
//        } finally {
...
//        }
        ...
        try {
            //create a Jsonobject from the given String SAMPLE_JSON_RESPONSE
            JSONObject rootjsonObject= new JSONObject(SAMPLE_JSON_RESPONSE);
            //Tracing the json Array with key "features"
            JSONArray jsonArray= rootjsonObject.optJSONArray("features");
            //Loop through all the features in the String
            for(int i=0; i<jsonArray.length(); i++){
                //get the objects in features
                JSONObject jsonObject= jsonArray.getJSONObject(i);
                //trace key "properties" in every object
                JSONObject properties= jsonObject.getJSONObject("properties");
                //Get magnitude, location and date in every "properties"
                double magnitude= properties.getDouble("mag");
                String location= properties.getString("place");
                long timeInMilliseconds = properties.getLong("time");
                String url= properties.getString("url");
                //Make a new Quake instance & add to the Arraylist
                Quake earthquake= new Quake(magnitude, location, timeInMilliseconds, url);
